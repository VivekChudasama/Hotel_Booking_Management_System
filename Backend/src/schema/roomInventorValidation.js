...
const validateCreateRoomInventory = [
    body('hotel_id').notEmpty().withMessage(ResponseMessages.room_inventory.HOTEL_ID_REQUIRED)
        .bail()
        .isMongoId().withMessage(ResponseMessages.hotel.VALID_HOTEL_ID),

    body('room_id').notEmpty().withMessage(ResponseMessages.room_inventory.ROOM_ID_REQUIRED)
        .bail()
        .isMongoId().withMessage(ResponseMessages.room.VALID_ROOM_ID),

    body('room_number').notEmpty().withMessage(ResponseMessages.room_inventory.ROOM_NUMBER_REQUIRED)
        .bail()
